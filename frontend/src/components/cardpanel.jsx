...
function Cardpanel(props) {
  ...
  useEffect(()=>{
    ...
    (async()=>{
      if (props.current_workspace!=null) {
        let cardsArray=[];
        let getcardsinfo = axios.post("https://server-2visqprab-monish062003s-projects.vercel.app/readworkspace",{
  ...
  const AddCard=(async()=>{
    ...
    setCount(count + 1);
    document.cookie=`count=${count} ; expires=Fri, 31 Dec 9999 23:59:59 GMT; path="/"`
    let response = await axios.post("https://server-2visqprab-monish062003s-projects.vercel.app/card",{
      email:document.cookie.split("=")[1],
